...
exports.seed = async function(knex) {
  ...
  await knex('GamFatura').del();
  await knex('GamSaaSConfig').del();
  ...
  // Insere Configurações Iniciais da Plataforma
  await knex('GamSaaSConfig').insert([
  ...
  await knex('GamEmpresa').insert({
    ...
    cor_primaria: '#D4AF37',
    status: 'ATIVO',
    data_expiracao: knex.raw("datetime('now', '+30 days')") // Expira em 30 dias por padrão
  });

  // Insere Usuário Exemplo (Super Admin)
  await knex('GamUsuario').insert({
    id: 's0s0s0s0-s0s0-s0s0-s0s0-s0s0s0s0s0s0',
    empresa_id: null,
    nome: 'Super Admin',
    email: 'super@plataforma.com.br',
